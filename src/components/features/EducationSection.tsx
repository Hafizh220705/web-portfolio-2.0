...
function EducationCard({
  ...
  return (
    <div
      ...
      className={`flex flex-col border-4 border-black bg-white p-5 md:p-6 shadow-neo
    ...
    >
      ...
      <div className="flex justify-between items-start mb-5">
        <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center border-4 border-black
          bg-white shadow-neo-sm -rotate-3 hover:rotate-0 transition-transform overflow-hidden"
        >
          {edu.logoUrl ? (
            <Image
              src={edu.logoUrl}
        ...
        <div className="border-2 border-black bg-neo-green px-3 py-1 shadow-neo-sm rotate-2 hover:rotate-0 transition-transform">
          <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-black/80">
            {edu.scoreLabel}
          </p>
          <p className="text-lg md:text-xl font-black uppercase leading-none tracking-tighter text-black mt-0.5">
            {edu.score}
          </p>
        </div>
      </div>
      ...
      {/* Title & school */}
      <div className="mb-4">
        <span className="inline-block border-2 border-black bg-white px-2 py-0.5 text-[9px] md:text-[10px]
          font-black uppercase mb-3 tracking-wider shadow-neo-sm"
        >
          {edu.duration}
        </span>
      ...
      <div className="pt-5 mt-2 border-t-2 border-dashed border-black/20 flex-grow">
        <ul className="space-y-2.5 pl-4 list-none font-jakarta text-[10px] md:text-xs
          font-semibold text-slate-800 leading-relaxed"
        >
  ...
  return (
    ...
    >
      <div ref={ref} className="mx-auto max-w-7xl px-4 lg:px-8 relative z-10">
        ...
        >
          <h2 className="inline-block border-4 border-black bg-neo-yellow px-8 py-3 text-4xl md:text-6xl
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
          {EDUCATION.map((edu, index) => (
            <EducationCard key={edu.school} edu={edu} index={index} inView={inView} />
          ))}
        </div>
